...
% This script performs 3D reconstruction of MRI data from raw k-space:
% 
% Part 1 - Multi-echo FLASH (reference/ground truth):
%   - Reads 3D k-space data with 5 echoes
%   - Performs 2D FFT slice-by-slice for each echo
%   - Combines coils using sum-of-squares
%
...
%   - ESPIRiT coil combination first
%   - Performs 2D POCS partial Fourier reconstruction (swapped order)
%   - 
%   - write DICOM
% 
...
% Extract dimensions
[Nx, Ny, Nz, Ncha_f, Necho] = size(d_srt);

% Reconstruct each echo using 2D FFT per slice (more memory efficient than 3D FFT)
img_sos = zeros(Nx, Ny, Nz, Necho, 'single');
for echo = 1:Necho
    for slc = 1:Nz
        % Apply 2D inverse FFT to each slice, then combine coils
        img_sos(:,:,slc,echo) = sos(ifft2c(d_srt(:,:,slc,:,echo)));
    end
end
...
%   - 3D acquisition: 128x128x128
%   - Partial Fourier: ~69% k-space coverage per F-state
...
figure(2)
img_pocs=ifftdim(k_fin,1:3);
sli=ceil(Nz/2);
img_pocs_sli=squeeze(img_pocs(:,:,sli,:,:));
img_pocs_sos = squeeze(sqrt(sum(abs(img_pocs_sli).^2,3)));
img_pocs_sos=reshape(img_pocs_sos,Nx,[]);
k_pocs_sli=fftdim(img_pocs_sos,1:2);
k_disp_sos=reshape(k_pocs_sli,Nx,[]);
imagesc((abs(img_pocs_sos))); colormap gray; axis image;
set(gca, 'XTick', [], 'YTick', []);
xlabel('From left to right: F0, F1, F2, F3, F4, respectively');
title('Sanity Check for POCS (in k-space)');

%% Extract Magnitude for Each F-state
fprintf('Combining coils across F-states...\n');
img_sos = zeros(Nx, Ny, Nz, N, 'single');    % Magnitude images
for f_idx = 1:N
    % Transform final k-space to image space (3D inverse FFT)
    complex_coil_imgs = ifftdim(k_fin(:,:,:,:,f_idx),1:3);
    
    % Magnitude: sum-of-squares across coils
    img_sos(:,:,:,f_idx) = squeeze(sqrt(sum(abs(complex_coil_imgs).^2,4)));
...
fprintf('Running ESPIRiT phase reconstruction...\n');

img_phase = zeros(Nx, Ny, Nz, N, 'single');  % Phase images
for f = 1:N
    for sli=1:Nz
        
        DATA_f_sli=squeeze(k_fin(:,:,sli,:,f)); % in k-space
        DATA = DATA_f_sli/max(max(max(abs(ifftdim(DATA_f_sli,1:2))))) + eps; % image space
        ...
        maps = M(:,:,:,end-1:end); %coils map estimate
        ...
        nIterCG = 12;
        % ESPIRiT CG reconstruction with 2 maps
        [reskESPIRiT, resESPIRiT] = cgESPIRiT(DATAc,ESP, nIterCG, 0.01,DATAc*0); % coils combined
        img_phase(:,:,sli,f)=resESPIRiT(:,:,end); % image domain
    end 
    disp('one F-state done')
end

fprintf('Combining coils across F-states...\n');
phase_maps = zeros(Nx, Ny, Nz, N, 'single');    % Magnitude images
for f_idx = 1:N
    % Magnitude: sum-of-squares across coils
    phase_maps(:,:,:,f_idx) = angle(img_phase(:,:,:,f_idx));    
end

%% Sanity check for phase (in k-space)

figure(3)
sli=ceil(Nz/2);
img_phase_sli=squeeze(phase_maps(:,:,sli,:));
img_phase_sli=reshape(img_phase_sli,Nx,[]);
imagesc(img_phase_sli); colormap jet; axis image; colorbar;
set(gca, 'XTick', [], 'YTick', []);
xlabel('From left to right: F0, F1, F2, F3, F4, respectively');
title('Sanity check for phase (in k-space)');

%% Export to DICOM Format (.IMA)
...
ssfp2dcm(prefix, img_sos, phase_maps, TE0_ms, TR, voxelSize_mm, B0_T, patientName, patientID, 001);

%% Save MATLAB Files
% Save reconstructed data (all F-states)
save(sprintf('img_sos_tr%d_n%d.mat', TR, N), 'img_sos', '-v7.3');
%save(sprintf('img_phase_tr%d_n%d.mat', TR, N), 'img_phase', '-v7.3');
