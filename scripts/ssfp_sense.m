...
%% SENSE recon 
ks_coilscombined=zeros(Nx,Ny,Nz,N);
for f=1:N
    img_coils=squeeze(img_pocs(:,:,:,:,f));
    sens = adaptive_est_sens(img_coils);
    tmp = SENSE_3D(img_coils,sens);
    ks_coilscombined(:,:,:,f) = fftdim(tmp,1:3);
end 

img_sos  = sqrt(sum(abs(img_pocs).^2,4));
S_0 = img_pocs./img_sos;
thresh = 0.05*max(abs(img_sos(:)));
mask = abs(img_sos) > thresh;
S_2 = S_0.*mask;
img_coils_combined = sum(img_pocs.*conj(S_2),4)./sum(S_2.*conj(S_2),4);
...
figure(3)
sli=ceil(Ny/2);
img_sli= squeeze(img_coils_combined(:,:,sli,:));
