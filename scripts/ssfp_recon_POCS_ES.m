...
%   - (optional) ESPIRiT/SENSE solve to obtain a coil-combined complex image and phase
%
...
%   - Combines coils using sum-of-squares (magnitude)
%   - ESPIRiT solve to obtain a coil-combined complex image and phase
% Note:
% We swapped the order of POCS and ESPIRiT to debug. 
...
ks_coilscombined = zeros(Nx, Ny, Nz, N);  % Phase images
for f = 1:N
    for sli=1:Nz
        
        DATA_f_sli = double(squeeze(d_fstates_n(:,:,sli,:,f)));   % <-- make double early
        DATA = DATA_f_sli / (max(max(max(abs(ifftdim(DATA_f_sli,1:2))))) + eps);
        
        ncalib = 24; ksize = [6,6]; eigThresh_k = 0.02; eigThresh_im = 0.9;
        [sx,sy,Nc] = size(DATA);
        ...
        DATAc = double(DATA .* mask);           % <-- double
        calib  = double(crop(DATAc,[sx,ncalib,Nc]));
        
        [k,S] = dat2Kernel(calib, ksize);
        idx   = max(find(S >= S(1)*eigThresh_k));
        
        [M,W] = kernelEig(k(:,:,:,1:idx), [sx,sy]);
        maps  = double(M(:,:,:,end-1:end));     % <-- double
        weights = double(W(:,:,end-1:end));
        weights = (weights - eigThresh_im)./(1-eigThresh_im).* (W(:,:,end-1:end) > eigThresh_im);
        weights = -cos(pi*weights)/2 + 1/2;
        
        ESP = ESPIRiT(maps, weights);
        
        nIterCG = 12; lambda = 0.01;
       
        [reskESPIRiT, resESPIRiT] = cgESPIRiT(DATAc, ESP, nIterCG, lambda, 0*DATAc);
        ks_coilscombined(:,:,sli,f) = reskESPIRiT(:,:,end-1);
...
% phase_maps = zeros(Nx, Ny, Nz, N, 'single');    
% for f_idx = 1:N
...
figure(2)
sli=ceil(Nz/2);
img_phase_sli=fftdim(squeeze(ks_coilscombined(:,:,sli,:)),1:3);
...
for f_idx=1:N
    for sli=1:Nz
        ...
        if f_idx>(cfg+1)  % Is F-state to left or right of centre?
            S(:,pe_lines,:)=squeeze(ks_coilscombined(:,1:n_lines,sli,f_idx));    
        else 
            S(:,pe_lines,:)=squeeze(ks_coilscombined(:,end-n_lines+1:end,sli,f_idx));    
        end
...
k_pocs_sli=fftdim(img_pocs_sli,1:2);
k_pocs_sli=reshape(k_pocs_sli,Nx,[]);
imagesc((angle(img_pocs_sli))); colormap jet; axis image;
set(gca, 'XTick', [], 'YTick', []);
xlabel('From left to right: F0, F1, F2, F3, F4, respectively');
...
ssfp2dcm(prefix, img_sos, img_phase, TE0_ms, TR, voxelSize_mm, B0_T, patientName, patientID, 001);
